...
#----  Prepare info (all variables to be included)

# Prepare `keep_cvars` vector 
nx <-21
...
cvars2    <- c("FEMALE","AGE")
keep_cvars <- c(BM_cvars, cvars0, cvars1, cvars2)
...
df_initInfo <- list(
   ...
   datain_script  = "060accord_olink_analytical_dataset040323",  # R script that creates data frames 
   datain_basename = datain_basename,
   datain_extension = datain_extension, 
   dfnms_all   = c("accord"),  # Data frames created by this script
   dfin_name   = "accord",     # One or two data frames (external training and/or validation) selected
   dfout_name  = c("accord_train", "accord_test"), # Data frames to be created
   keep_vars   = keep_cvars,
   CCH_data    = TRUE,
   id          = "MASKID",
   tvars_all   = tvars_all,    # Matrix with pairs of variables used to create Surv objects
   cfilter     = character(0), # Place holder 
   cfilter_comment = "No filter applied to data",
   time_horizon = Inf,         # Inf -> no time truncation 
   initSplit    = 0.8,         # numeric(0) -> No split
   nfolds       = 10
)
...
keep_objects <- c("accord", "df_initInfo", "dfCCH_initInfo")
